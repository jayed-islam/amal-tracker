...
// // ─────────────────────────────────────────────────────────────────────────────
// // DIALOG STATE  — tracks async lifecycle
// // ─────────────────────────────────────────────────────────────────────────────

// enum _DialogPhase { idle, loading, success, error }

// // ─────────────────────────────────────────────────────────────────────────────
// // CONFIRM DIALOG
// // Stays open during API call. Shows inline loading → success → error.
// // Only auto-closes on success (after brief feedback). Error stays visible.
// // ─────────────────────────────────────────────────────────────────────────────
...
//   /// Must return a Future. Throw to signal error.
//   final Future<void> Function() onConfirm;
//   final String successMessage;
//   final String? errorMessage; // null → use caught message
...
//     required this.onConfirm,
//     this.showCancel = true,
//     this.successMessage = 'সফলভাবে আপডেট হয়েছে',
//     this.errorMessage,
//   });
...
//     if (_phase == _DialogPhase.loading) return;
//     HapticFeedback.selectionClick();
//     setState(() => _phase = _DialogPhase.loading);
...
//       // Auto-close after user sees success feedback
...
//         _errorText = widget.errorMessage ?? e.toString();
//       });
//       HapticFeedback.mediumImpact();
//       _feedbackCtrl.forward(from: 0);
//     }
...
//   @override
//   Widget build(BuildContext context) {
//     final isLoading = _phase == _DialogPhase.loading;
//     final isSuccess = _phase == _DialogPhase.success;
//     final isError = _phase == _DialogPhase.error;
...
//                                 child: isLoading
//                                     ? Text('আপডেট হচ্ছে...',
//                                         key: const ValueKey('lbl_loading'),
//                                         style: const TextStyle(
//                                             color: _C.textHint, fontSize: 11.5))
//                                     : const SizedBox.shrink(
//                                         key: ValueKey('lbl_empty')),
//                               ),
//                             ],
//                           ),
//                         ),
...
//               SizedBox(
//                 height: 52,
//                 child: _buildActions(isLoading, isSuccess, isError),
//               ),
//             ],
//           ),
//         ),
...
//   Widget _buildActions(bool isLoading, bool isSuccess, bool isError) {
//     // ── Success: single "বন্ধ করুন" ──
//     if (isSuccess) {
//       return _ActionButton(
//         label: 'বন্ধ করুন',
//         color: _C.green,
//         onTap: () => Navigator.of(context).pop(true),
//         position: _ActionPosition.single,
//       );
//     }
...
//             position: _ActionPosition.right,
//             bold: true,
//           ),
//         ),
//       ]);
...
//     // ── Idle: cancel + confirm  OR  single confirm ──
...
// // ─────────────────────────────────────────────────────────────────────────────
// // ACTION BUTTON  — bottom strip buttons
// // ─────────────────────────────────────────────────────────────────────────────

// enum _ActionPosition { left, right, single }
...
//     BorderRadius? br;
//     switch (position) {
//       case _ActionPosition.left:
//         br = const BorderRadius.only(bottomLeft: Radius.circular(22));
//         break;
//       case _ActionPosition.right:
//         br = const BorderRadius.only(bottomRight: Radius.circular(22));
//         break;
//       case _ActionPosition.single:
...
// // ─────────────────────────────────────────────────────────────────────────────
// // FEEDBACK BANNER  — shown inside dialog on success / error
// // ─────────────────────────────────────────────────────────────────────────────
...
//         .slideY(begin: 0.08, duration: 220.ms, curve: Curves.easeOut);
//   }
// }

// // ─────────────────────────────────────────────────────────────────────────────
// // HELPER — show dialog and return result
// // ─────────────────────────────────────────────────────────────────────────────
...
//   String successMessage = 'সফলভাবে আপডেট হয়েছে',
//   String? errorMessage,
// }) {
...
//   // ──────────────────────────────────────────────────────────────────────────
//   // TOGGLE HANDLERS  — each opens dialog with proper async onConfirm
//   // ──────────────────────────────────────────────────────────────────────────
...
//       onConfirm: () async {
//         await ref.read(privacyProvider.notifier).updatePrivacy(
//               isPermanent: newVal,
//               isHidden: privacy.isHidden,
//               showAnonymous: privacy.showAnonymous,
//             );
...
//     // Already locked out this month — info-only dialog
//     if (!newVal && privacy.canRejoinThisMonth == false) {
//       await _showConfirmDialog(
//         context,
//         icon: Icons.lock_clock_rounded,
...
//       confirmText: newVal ? 'নাম লুকাই' : 'নাম দেখাই',
//       confirmColor: _C.teal,
//       successMessage: newVal ? 'নাম লুকানো হয়েছে' : 'নাম দেখানো হচ্ছে',
//       onConfirm: () async {
//         await ref.read(privacyProvider.notifier).updatePrivacy(
//               isPermanent: privacy.isPermanent,
//               isHidden: privacy.isHidden,
...
//       onConfirm: () async {
//         await ref.read(privacyProvider.notifier).toggleProfileShare(newVal);
//       },
//     );
//   }
// }
...
//       confirmText: 'হ্যাঁ, মুছুন',
//       confirmColor: _C.red,
//       successMessage: 'অ্যাকাউন্ট মুছে ফেলা হয়েছে',
//       errorMessage: 'অ্যাকাউন্ট মুছতে সমস্যা হয়েছে। আবার চেষ্টা করুন।',
//       onConfirm: () async {
//         // TODO: await deleteAccountApi();
...
// // ─────────────────────────────────────────────────────────────────────────────
// // DIALOG RESULT
// // The dialog ONLY runs the raw API call.
// // Provider state is updated by the CALLER after success.
// // This ensures the switch never flips until the full round-trip completes.
// // ─────────────────────────────────────────────────────────────────────────────

// class _DialogResult {
//   final bool confirmed; // false = user cancelled
//   final bool success; // only meaningful when confirmed == true
//   final String error;
...
// // ─────────────────────────────────────────────────────────────────────────────
// // Lifecycle:
// //   idle    → user reads the prompt, taps confirm or cancel
// //   loading → API call in flight; back button blocked, buttons hidden
// //   (done)  → dialog pops immediately; caller shows snackbar
// //
// // The switch value (from Riverpod) is NOT touched here.
// // Caller updates the provider only after we return _DialogResult.ok().
// // ─────────────────────────────────────────────────────────────────────────────
...
//   /// Raw API call — must throw on failure.
//   /// Do NOT update any Riverpod provider here.
...
//       if (mounted) Navigator.of(context).pop(const _DialogResult.ok());
//     } catch (e) {
//       if (mounted) {
...
// // ─────────────────────────────────────────────────────────────────────────────
// // SETTINGS PROVIDER  (non-notification prefs)
// // ─────────────────────────────────────────────────────────────────────────────
...
//   // HANDLERS
//   //
//   // Pattern for every toggle:
//   //   1. Open dialog — apiCall = raw API only, NO provider update inside
//   //   2. Dialog returns _DialogResult after closing
//   //   3. If success  → update provider (switch flips HERE) → success snackbar
//   //   4. If api fail → switch stays unchanged           → error snackbar
//   //   5. If cancelled → nothing changes
//   // ──────────────────────────────────────────────────────────────────────────
...
//       confirmText: newVal ? 'গোপন রাখব' : 'ফিরব',
//       confirmColor: _C.purple,
//       // ⬇ raw API call only — no provider update here
...
//     if (result.success) {
//       // ⬇ provider update happens HERE — after dialog closes → switch flips now
//       ref.read(privacyProvider.notifier).updatePrivacy(
//             isPermanent: newVal,
//             isHidden: privacy.isHidden,
//             showAnonymous: privacy.showAnonymous,
...
//       _showSnack(
//         context,
//         newVal ? 'আমল গোপন রাখা হয়েছে' : 'পরের মাস থেকে লিডারবোর্ডে দেখা যাবে',
//       );
//     } else if (result.confirmed) {
//       // API failed — switch stays unchanged
//       _showSnack(
//         context,
...
//   Future<void> _onMonthlyToggle(PrivacyState privacy, bool newVal) async {
//     // Already locked out this month — info-only, no API call
//     if (!newVal && privacy.canRejoinThisMonth == false) {
//       await _showConfirm(
//         context,
//         icon: Icons.lock_clock_rounded,
...
//         confirmColor: _C.amber,
//         showCancel: false,
//         apiCall: () async {}, // info-only — no call
//       );
//       return;
//     }
...
//       _showSnack(
//         context,
//         newVal ? 'এই মাস থেকে বাদ দেওয়া হয়েছে' : 'এই মাসে যোগ দেওয়া হয়েছে',
//       );
//     } else if (result.confirmed) {
//       _showSnack(
//         context,
...
//       _showSnack(
//         context,
//         newVal ? 'নাম লুকানো হয়েছে' : 'নাম দেখানো হচ্ছে',
//       );
//     } else if (result.confirmed) {
//       _showSnack(
//         context,
...
//       _showSnack(
//         context,
//         newVal ? 'প্রোফাইল সবার জন্য খোলা হয়েছে' : 'প্রোফাইল বন্ধ করা হয়েছে',
//       );
//     } else if (result.confirmed) {
//       _showSnack(
//         context,
...
// // These must throw on failure — the dialog catches throws as errors.
// // ─────────────────────────────────────────────────────────────────────────────

// class PrivacyApi {
//   static Future<void> updatePermanent(bool val) async {
//     // await YourApiService.patch('/privacy', {'isPermanent': val});
//     await Future.delayed(const Duration(seconds: 1)); // placeholder
//   }

//   static Future<void> updateHidden(bool val) async {
//     // await YourApiService.patch('/privacy', {'isHidden': val});
//     await Future.delayed(const Duration(seconds: 1));
//   }

//   static Future<void> updateAnonymous(bool val) async {
//     // await YourApiService.patch('/privacy', {'showAnonymous': val});
//     await Future.delayed(const Duration(seconds: 1));
//   }

//   static Future<void> updateProfileShare(bool val) async {
//     // await YourApiService.patch('/privacy', {'isPublic': val});
//     await Future.delayed(const Duration(seconds: 1));
//   }

//   static Future<void> deleteAccount() async {
//     // await YourApiService.delete('/account');
//     await Future.delayed(const Duration(seconds: 1));
//   }
// }
...
// // ─────────────────────────────────────────────────────────────────────────────
// // PRIVACY TOGGLE TILE
// // value is always read from Riverpod — never a local copy
// // ─────────────────────────────────────────────────────────────────────────────
...
//       opacity: disabled ? 0.4 : 1.0,
//       child: GestureDetector(
//         // Opens dialog — does NOT flip value directly
//         onTap: disabled ? null : () => onToggle(!value),
//         behavior: HitTestBehavior.opaque,
//         child: Padding(
//           padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 13),
...
//             ),
//             const SizedBox(width: 8),
//             // IgnorePointer so the Switch itself doesn't intercept taps —
//             // the parent GestureDetector opens the dialog instead.
//             IgnorePointer(
//               child: Switch.adaptive(
//                 value: value, // always the Riverpod value — never optimistic
//                 onChanged: disabled ? null : (_) {},
//                 activeColor: activeColor,
//                 activeTrackColor: activeTrackColor,
//                 inactiveThumbColor: _C.textHint,
...
//       confirmText: 'হ্যাঁ, মুছুন',
//       confirmColor: _C.red,
//       apiCall: () => PrivacyApi.deleteAccount(),
//     );

//     if (!context.mounted) return;

//     if (result.success) {
//       // TODO: sign out and navigate to onboarding
...
// // ─────────────────────────────────────────────────────────────────────────────
// // DIALOG RESULT  — what the caller receives after the dialog closes
// // ─────────────────────────────────────────────────────────────────────────────

// class _DialogResult {
//   final bool confirmed; // false = user cancelled
//   final bool success; // true = API succeeded
//   final String error;
...
// //  Execution order:
// //    1. User taps confirm
// //    2. Spinner shows (dialog stays open, back blocked)
// //    3. apiCall() runs  ──► throws = error path
// //    4. onSuccess()     ← provider update runs HERE (switch flips while
// //                           dialog is still on screen / animating out)
// //    5. Navigator.pop(_DialogResult.ok())
// //    6. Caller shows snackbar
// //
// //  This means the switch state is already correct by the time the
// //  dialog finishes closing — zero visible lag.
// // ─────────────────────────────────────────────────────────────────────────────
...
//   /// Step 1: raw network call only — must throw on failure.
//   final Future<void> Function() apiCall;

//   /// Step 2: called synchronously after apiCall() succeeds, before pop.
//   /// Put provider / local-state updates here so the UI reflects the new
//   /// value while the dialog is still animating out.
...
//       if (!mounted) return;
//       widget.onSuccess(); // ← provider update (sync, instant)
//       Navigator.of(context).pop(const _DialogResult.ok());
//     } catch (e) {
//       if (!mounted) return;
//       // Do NOT call onSuccess — leave provider untouched so switch stays as-is
//       Navigator.of(context).pop(_DialogResult.fail(e.toString()));
//     }
//   }
...
//   // HANDLERS
//   //
//   //  apiCall   = network only, throws on failure
//   //  onSuccess = provider.update() — runs inside dialog after API succeeds,
//   //              BEFORE Navigator.pop → switch is already correct when dialog closes
//   //
//   //  After _showConfirm returns:
//   //    result.success   → show success snackbar (switch already flipped)
//   //    result.confirmed → show error snackbar   (switch unchanged)
//   //    cancelled        → do nothing
//   // ──────────────────────────────────────────────────────────────────────────
...
//       confirmColor: _C.purple,
//       apiCall: () => PrivacyApi.updatePermanent(newVal),
//       // ↓ runs after API success, BEFORE dialog pop → instant switch flip
//       onSuccess: () => ref.read(privacyProvider.notifier).updatePrivacy(
//             isPermanent: newVal,
//             isHidden: privacy.isHidden,
//             showAnonymous: privacy.showAnonymous,
//           ),
...
//   Future<void> _onMonthlyToggle(PrivacyState privacy, bool newVal) async {
//     // Locked out this month — info only, no API call, no onSuccess needed
//     if (!newVal && privacy.canRejoinThisMonth == false) {
//       await _showConfirm(
//         context,
//         icon: Icons.lock_clock_rounded,
...
// // ─────────────────────────────────────────────────────────────────────────────
// // PRIVACY API STUB  — replace with your actual service
// // Must throw on failure so the dialog catches it as an error.
// // ─────────────────────────────────────────────────────────────────────────────
...
// // ─────────────────────────────────────────────────────────────────────────────
// // PRIVACY TOGGLE TILE
// // Switch value always comes from Riverpod — never a local copy.
// // IgnorePointer on the Switch means only the GestureDetector (whole row) fires.
// // ─────────────────────────────────────────────────────────────────────────────
...
//       confirmColor: _C.red,
//       apiCall: () => PrivacyApi.deleteAccount(),
//       onSuccess: () {}, // TODO: sign out / clear local state here
//     );
...
// ─────────────────────────────────────────────────────────────────────────────
// PRIVACY API STUB  — replace with your actual service calls
// ─────────────────────────────────────────────────────────────────────────────
