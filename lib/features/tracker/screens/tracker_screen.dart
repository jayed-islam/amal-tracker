...
//     // ── No entry yet: single add button ─────────────────────────────────────
//     if (!hasEntry) {
//       return GestureDetector(
//         onTap: isSaving ? null : onAdd,
//         child: Container(
...
class _TrackerScreenState extends ConsumerState<TrackerScreen> {
  ...
  //
  //  Old (broken):
  //    onConfirm → pop dialog immediately → run API in background
  //
  //  New (fixed):
  //    Dialog opens → user confirms → spinner shows inside dialog →
  //    API call runs → dialog pops with result → snackbar shown by caller
  // ──────────────────────────────────────────────────────────────────────────
  ...
  Future<void> _confirmDelete(BuildContext context, String dateStr) async {
    ...
    // _DeleteDialog returns true on success, false on API failure, null on cancel
    final result = await showDialog<bool>(
      context: context,
      barrierDismissible: false,
      builder: (_) => _DeleteDialog(
        // ↓ raw API call only — dialog handles loading state
        onDelete: () =>
...
//
//  Lifecycle:
//    idle    → shows icon + message + বাতিল/মুছুন buttons
//    loading → buttons replaced by spinner; back button blocked
//    done    → pops with true (success) or false (API error)
//              caller shows snackbar and refreshes list
//
// ─────────────────────────────────────────────────────────────────────────────

class _DeleteDialog extends StatefulWidget {
  /// Raw API call. Must return true on success, false on failure.
  /// Do NOT pop the dialog inside this callback.
...
class _DeleteDialogState extends State<_DeleteDialog> {
  ...
  Future<void> _handleConfirm() async {
    ...
    try {
      ...
      if (mounted) Navigator.of(context).pop(ok); // true or false
    } catch (_) {
...
// ─────────────────────────────────────────────────────────────────────────────
// DIALOG BUTTON  (shared by _DeleteDialog)
// ─────────────────────────────────────────────────────────────────────────────
