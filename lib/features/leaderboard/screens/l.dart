...
//   // Same pattern as month: a compact trigger up top opens a sheet, instead
//   // of a permanently-visible filter row taking its own slice of the screen.
//   void _pickGender() {
//     final f = ref.read(leaderboardFilterProvider);
//     showModalBottomSheet(
//       context: context,
...
//         selected: f.gender,
//         onPicked: (g) {
//           final next = f.copyWith(gender: g, clearGender: g == null, page: 1);
//           ref.read(leaderboardFilterProvider.notifier).state = next;
//           ref.read(leaderboardProvider.notifier).load(next, refresh: true);
//         },
//       ),
...
//                 slivers: [
//                   // ── App Bar ─────────────────────────────────────────────
//                   // Both filters (month, gender) live here as compact
//                   // trigger buttons that open a picker sheet — one control
//                   // per filter, always visible at the top, nothing repeated
...
//                         onTap: _pickMonth,
//                         child: Container(
//                           margin: EdgeInsets.only(right: isMaleUser ? 16 : 8),
//                           padding: const EdgeInsets.symmetric(
...
//                         ),
//                       ),
//                       // Male users only ever see male profiles (no gender
//                       // choice actually applies to them), so this button —
//                       // like the row it replaces — only shows up when it
//                       // does something.
//                       if (!isMaleUser)
//                         GestureDetector(
//                           onTap: _pickGender,
...
// // ─────────────────────────────────────────────────────────────────────────────
// // HERO BAND — points বাদ, completion % + farz + jamaat + streak
// // month/year আর এখানে repeat করা হয় না — AppBar-এর month button-ই এখন
// // display ও control-এর একমাত্র জায়গা।
// // ─────────────────────────────────────────────────────────────────────────────
...
// // ─────────────────────────────────────────────────────────────────────────────
// // GENDER PICKER SHEET — same sheet pattern as month, just for gender.
// // Reuses the existing GenderFilterChips widget so the chip styling stays
// // consistent with wherever else it's used in the app; this sheet only
// // changes WHERE it's shown (a triggered sheet instead of an always-visible
// // row) and closes itself once a choice is made.
// // ─────────────────────────────────────────────────────────────────────────────

// class _GenderPickerSheet extends StatelessWidget {
//   final String? selected;
//   final void Function(String?) onPicked;
//   const _GenderPickerSheet({required this.selected, required this.onPicked});
...
//                 borderRadius: BorderRadius.circular(99))),
//         const SizedBox(height: 20),
//         Text('জেন্ডার অনুযায়ী ফিল্টার করুন',
//             style: TextStyle(
//                 color: context.colors.textPrimary,
//                 fontSize: 16,
//                 fontWeight: FontWeight.w700)),
...
class _LeaderboardScreenState extends ConsumerState<LeaderboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // While a full load/refresh is in flight, the month & gender buttons
    // shouldn't be tappable — starting another load mid-flight can race
    // with the one already running and leave the filter state and the
    // visible list out of sync.
    final isFilterBusy = state.isLoading || state.isRefreshing;
              ...
              child: CustomScrollView(
                ...
                slivers: [
                  ...
                  SliverAppBar(
                    ...
                    actions: [
                      GestureDetector(
                        onTap: isFilterBusy ? null : _pickMonth,
                        child: Opacity(
                          opacity: isFilterBusy ? 0.45 : 1,
                          child: Container(
                            margin: EdgeInsets.only(right: isMaleUser ? 16 : 8),
                            padding: const EdgeInsets.symmetric(
                                horizontal: 10, vertical: 7),
                      ...
                      if (!isMaleUser)
                        GestureDetector(
                          onTap: isFilterBusy ? null : _pickGender,
                          child: Opacity(
                            opacity: isFilterBusy ? 0.45 : 1,
                            child: Container(
                              margin: const EdgeInsets.only(right: 16),
...
// ─────────────────────────────────────────────────────────────────────────────
// GENDER PICKER SHEET — same sheet pattern as month, just for gender.
// Self-contained option list (not delegating to the external
// GenderFilterChips widget) so selection is guaranteed to actually apply.
// ─────────────────────────────────────────────────────────────────────────────
...
class _GenderPickerSheet extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // Self-contained options instead of embedding the external
    // GenderFilterChips widget — that widget wasn't actually applying the
    // selection when used inside this sheet, so this list owns its own
    // tap-to-select-and-close logic end to end.
    final options = <(String? value, String label, IconData icon)>[
      (null, 'সবাই', Icons.groups_rounded),
      ('male', 'পুরুষ', Icons.male_rounded),
      ('female', 'মহিলা', Icons.female_rounded),
    ];
            ...
            child: GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                child: Row(children: [
                  ...
                  if (isSelected)
                    Icon(Icons.check_circle_rounded,
                        size: 18, color: context.colors.darkGreen),
                ]),
              ),
            ),
          );
