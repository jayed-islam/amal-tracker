...
//     if (!refresh &&
//         _lastLoadedAt != null &&
//         DateTime.now().difference(_lastLoadedAt!).inSeconds < 60 &&
//         filter.page == 1) {
...
// // ১. অটো রিলিজের জন্য জাস্ট এটিকে .autoDispose করে দেওয়া হয়েছে
// final leaderboardFilterProvider =
//     StateProvider.autoDispose<LeaderboardFilter>((ref) {
//   final now = DateTime.now();
//   final user = ref.watch(currentUserProvider);
...
// // ২. মেইন লিডারবোর্ড প্রোভাইডার (ফিল্টার ওয়াচ করবে এবং ফার্স্ট লোড অটো হ্যান্ডেল করবে)
// final leaderboardProvider =
//     StateNotifierProvider.autoDispose<LeaderboardNotifier, LeaderboardState>(
//   (ref) {
//     final api = ref.read(apiServiceProvider);
...
//     final notifier = LeaderboardNotifier(api);
//     notifier.load(filter);
//     return notifier;
//   },
// );

// final leaderboardPreviewProvider =
//     Provider.autoDispose<LeaderboardState>((ref) {
//   final mainState = ref.watch(leaderboardProvider);
//   final previewEntries = mainState.entries.take(3).toList();
//   return mainState.copyWith(entries: previewEntries);
// });
...
// // ৫. টাইপ আগের মতোই রেকর্ড ({int year, int month}) রাখা হয়েছে। কোনো চেঞ্জ নাই!
// final winnersProvider =
//     FutureProvider.autoDispose.family<List<dynamic>, ({int year, int month})>(
//   (ref, params) async {
//     final api = ref.read(apiServiceProvider);
...
// // ৬. টাইপ আগের মতোই রেকর্ড ({String userId, int year, int month}) রাখা হয়েছে। কোনো চেঞ্জ নাই!
// final publicProfileProvider = FutureProvider.autoDispose
//     .family<PublicMonthlyDetail, ({String userId, int year, int month})>(
//   (ref, params) async {
//     final api = ref.read(apiServiceProvider);
...
//   @override
//   int get hashCode =>
//       year.hashCode ^ month.hashCode ^ page.hashCode ^ limit.hashCode ^ gender.hashCode;
// }
...
//       completionPercentage:  (currentMonth?['completionPercentage'] as num?)?.toDouble() ?? 0,
//       farzCompletedDays:     (currentMonth?['farzCompletedDays'] as num?)?.toInt() ?? 0,
//       congregationDaysTotal: (currentMonth?['congregationDaysTotal'] as num?)?.toInt() ?? 0,
...
//       final hasMore = newEntries.length >= limit &&
//           (page * limit) < total;
...
//     PublicMonthlyDetail,
//     ({String userId, int year, int month})>((ref, params) async {
//   final api = ref.read(apiServiceProvider);
//   final res = await api.get<Map<String, dynamic>>(
//     '/tracker/profile/${params.userId}/${params.year}/${params.month}',
//   );
...
// final leaderboardFilterProvider =
//     StateProvider<LeaderboardFilter>((ref) {
//   final now = DateTime.now();
